    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (isCreate) {
          ...
          if (
            values.studentClass ===
            currentStudent["class_grade"] + currentStudent["class_name"]
          ) {
            values.studentClass = currentStudent["class_id"].toString();
          }
          values.studentImg = values.studentImg[0]
            ? values.studentImg[0]
            : values.studentImg.file;
          editStudent(values);
          this.props.form.resetFields();
        }
      }
      ...
      <div>
        ...
        >
          <Form>
            ...
            <FormItem label="学生头像" {...formItemLayout}>
            ...
            <FormItem label="选择班级" {...formItemLayout}>
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    handleStudentModal(studentModalVisible) {
      dispatch({
        type: "student/handleStudentModal",
        payload: { studentModalVisible }
      });
    },
    fetchClassList() {
    ...
    editStudent(currentEditStudent) {
      dispatch({
        type: "student/editStudent",
        payload: { currentEditStudent }
      });
    }
  };
}
