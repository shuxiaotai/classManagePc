...
class CourseScreen extends Component {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (isCreate) {
          ...
          values.courseImg = values.courseImg[0]
            ? values.courseImg[0]
            : values.courseImg.file;
          editCourse(values);
          this.props.form.resetFields();
        }
      }
  ...
  render() {
    ...
    const uploadProps = {
      action: `${getProtocol()}/api/addCourse`,
      listType: "picture",
      ...
      <div>
        ...
        >
          <Form>
            ...
            <FormItem label="课程头像" {...formItemLayout}>
            ...
            {isCreate ? (
              <FormItem label="默认课程" {...formItemLayout}>
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    handleCourseModal(courseModalVisible) {
      dispatch({
        type: "course/handleCourseModal",
        payload: { courseModalVisible }
      });
    },
    deleteCourse(id) {
