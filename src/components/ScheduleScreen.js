...
class ScheduleScreen extends Component {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (isCreate) {
          ...
          values.scheduleImg = values.scheduleImg[0]
            ? values.scheduleImg[0]
            : values.scheduleImg.file;
          editSchedule(values);
          this.props.form.resetFields();
        }
      }
  ...
  render() {
    ...
    const uploadProps = {
      action: `${
        isCreate
          ? `${getProtocol()}/api/addSchedule`
          : `${getProtocol()}/api/editSchedule`
      }`,
      listType: "picture",
    ...
    return (
      <div>
        ...
        >
          <Form>
            <FormItem label="日程名称" {...formItemLayout}>
            ...
            <FormItem label="日程头像" {...formItemLayout}>
            ...
            {isCreate ? (
              <FormItem label="默认日程" {...formItemLayout}>
  ...
  return {
    ...
    addSchedule(currentAddSchedule) {
      dispatch({
        ...
        payload: { currentAddSchedule }
      });
    },
    handleScheduleModal(scheduleModalVisible) {
      dispatch({
        type: "schedule/handleScheduleModal",
        payload: { scheduleModalVisible }
      });
    },
    deleteSchedule(id) {
