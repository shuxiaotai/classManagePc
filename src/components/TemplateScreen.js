...
class TemplateScreen extends Component {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (isCreate) {
          ...
          values.isPraise = templateType;
          values.templateImg = values.templateImg[0]
            ? values.templateImg[0]
            : values.templateImg.file;
  ...
  render() {
    ...
    const uploadProps = {
      action: `${
        isCreate
          ? `${getProtocol()}/api/addTemplate`
          : `${getProtocol()}/api/editTemplate`
      }`,
      listType: "picture",
        ...
        render: (text, record) => (
          <span>
            <Popconfirm
              title="确认修改?"
              onConfirm={() =>
                this.handleEdit(record.id, this.state.templateType)
              }
              okText="确认"
              cancelText="取消"
            >
            ...
            <Popconfirm
              title="确认删除?"
              onConfirm={() =>
                this.handleDelete(record.id, this.state.templateType)
              }
              okText="确认"
              cancelText="取消"
            >
    ...
    return (
      <div style={{ position: "relative" }}>
        ...
        <div style={{ position: "absolute", right: 10, top: 10 }}>
          当前为{templateType === true ? "表扬" : "批评"}模板
        </div>
        <Table
          columns={columns}
        ...
        >
          <Form>
            ...
            <FormItem label="模板头像" {...formItemLayout}>
            ...
            {isCreate ? (
              <FormItem label="默认点评" {...formItemLayout}>
  ...
  return {
    ...
    addTemplate(currentAddTemplate) {
      dispatch({
        ...
        payload: { currentAddTemplate }
      });
    },
    handleTemplateModal(templateModalVisible) {
      dispatch({
        type: "template/handleTemplateModal",
        payload: { templateModalVisible }
      });
    },
    deleteTemplate(id, isPraise) {
