...
export default {
  ...
  effects: {
    ...
    *addSchedule({ payload }, { call, put }) {
      let data = yield call(scheduleServices.addSchedule, {
        currentAddSchedule: payload.currentAddSchedule
      });
    ...
    *deleteSchedule({ payload }, { call, put }) {
      let data = yield call(scheduleServices.deleteSchedule, {
        id: payload.id
      });
    ...
    *fetchCurrentSchedule({ payload }, { call, put }) {
      let data = yield call(scheduleServices.fetchCurrentSchedule, {
        id: payload.id
      });
    ...
    *editSchedule({ payload }, { call, put }) {
      let data = yield call(scheduleServices.editSchedule, {
        currentEditSchedule: payload.currentEditSchedule
      });
      yield put({ type: "handleScheduleModal", payload: data });
  ...
  reducers: {
    ...
    handleScheduleModal(state, action) {
      return {
        ...state,
        scheduleModalVisible: action.payload.addScheduleSuccess
          ? !action.payload.addScheduleSuccess
          : action.payload.scheduleModalVisible
      };
    },
