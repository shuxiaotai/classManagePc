...
export default {
  ...
  effects: {
    *fetchTemplateList({ payload }, { call, put }) {
      let data = yield call(templateServices.fetchTemplateList, {
        isPraise: payload.isPraise
      });
      yield put({ type: "saveTemplateList", payload: data });
    },
    *addTemplate({ payload }, { call, put }) {
      let data = yield call(templateServices.addTemplate, {
        currentAddTemplate: payload.currentAddTemplate
      });
      ...
      if (data.addTemplateSuccess) {
        ...
        yield put({
          ...
          payload: { isPraise: payload.currentAddTemplate.isPraise ? 0 : 1 }
        });
      }
    },
    *deleteTemplate({ payload }, { call, put }) {
      let data = yield call(templateServices.deleteTemplate, {
        id: payload.id
      });
      if (data.deleteTemplateSuccess) {
        ...
        yield put({
          type: "fetchTemplateList",
          payload: { isPraise: payload.isPraise ? 0 : 1 }
        });
      }
    },
    *fetchCurrentTemplate({ payload }, { call, put }) {
      let data = yield call(templateServices.fetchCurrentTemplate, {
        id: payload.id
      });
    ...
    *editTemplate({ payload }, { call, put }) {
      let data = yield call(templateServices.editTemplate, {
        currentEditTemplate: payload.currentEditTemplate
      });
      yield put({ type: "handleTemplateModal", payload: data });
      if (data.editTemplateSuccess) {
        ...
        yield put({
          type: "fetchTemplateList",
          payload: { isPraise: payload.currentEditTemplate.isPraise ? 0 : 1 }
        });
      }
    }
  },
  ...
  reducers: {
    ...
    handleTemplateModal(state, action) {
      return {
        ...state,
        templateModalVisible: action.payload.addTemplateSuccess
          ? !action.payload.addTemplateSuccess
          : action.payload.templateModalVisible
      };
    },
