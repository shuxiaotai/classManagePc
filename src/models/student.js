...
export default {
  ...
  effects: {
    ...
    *addStudent({ payload }, { call, put }) {
      let data = yield call(studentServices.addStudent, {
        currentAddStudent: payload.currentAddStudent
      });
    ...
    *fetchCurrentStudent({ payload }, { call, put }) {
      let data = yield call(studentServices.fetchCurrentStudent, {
        id: payload.id
      });
    ...
    *editStudent({ payload }, { call, put }) {
      let data = yield call(studentServices.editStudent, {
        currentEditStudent: payload.currentEditStudent
      });
      yield put({ type: "handleStudentModal", payload: data });
  ...
  reducers: {
    ...
    handleStudentModal(state, action) {
      return {
        ...state,
        studentModalVisible: action.payload.addStudentSuccess
          ? !action.payload.addStudentSuccess
          : action.payload.studentModalVisible
      };
    },
